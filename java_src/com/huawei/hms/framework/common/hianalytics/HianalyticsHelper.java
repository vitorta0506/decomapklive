...
public class HianalyticsHelper {
    ...
    private void onNewEvent(Context context, String str, Map map, int i9) {
        if (context == null || map == null) {
            return;
        }
        Logger.v(TAG, "data = %s", map);
        try {
            HiAnalyticsUtils.getInstance().onNewEvent(context, str, map, i9);
    ...
    public void reportData(Context context, LinkedHashMap<String, String> linkedHashMap, String str, int i9) {
        if (isEnableReportNoSeed(context)) {
            onEvent(linkedHashMap, str, i9);
        }
    }
    ...
    public void onEvent(LinkedHashMap<String, String> linkedHashMap, String str, int i9) {
        HiAnalyticsInstance hiAnalyticsInstance;
        if (this.hasHMSBI) {
            onNewEvent(ContextHolder.getAppContext(), str, linkedHashMap, i9);
        } else if (i9 == 0) {
            Logger.v(TAG, "the base sdk isn't exsit, and reportType is %s", Integer.valueOf(i9));
            return;
        }
