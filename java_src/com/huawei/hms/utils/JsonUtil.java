...
public class JsonUtil {
        ...
        if (objectValue != null) {
            try {
                ...
                if ((objectValue instanceof JSONObject) && ((JSONObject) objectValue).has(VAL_TYPE)) {
                    int i9 = ((JSONObject) objectValue).getInt(VAL_TYPE);
                    if (i9 != 1 && i9 != 0) {
                        if (i9 == 2) {
                        ...
                        HMSLog.e(TAG, "cannot support type : " + i9);
                    }
    ...
    private static List<Object> readList(Type type, JSONObject jSONObject) throws JSONException, IllegalAccessException, InstantiationException {
        int i9 = jSONObject.getInt(LIST_SIZE);
        int i10 = jSONObject.getInt(VAL_TYPE);
        ArrayList arrayList = new ArrayList(i9);
        for (int i11 = 0; i11 < i9; i11++) {
            Object obj = jSONObject.get(LIST_ITEM_VALUE + i11);
            if (i10 == 0) {
    ...
    private static Map readMap(Type type, JSONObject jSONObject) throws JSONException, IllegalAccessException, InstantiationException {
        ...
        for (int i9 = 0; i9 < jSONArray.length(); i9 += 2) {
            if (cls.newInstance() instanceof IMessageEntity) {
                hashMap.put(jSONArray.get(i9), jsonToEntity(jSONArray.getString(i9 + 1), (IMessageEntity) cls.newInstance()));
            } else {
                hashMap.put(jSONArray.get(i9), jSONArray.get(i9 + 1));
            }
        }
        return hashMap;
    }
    ...
    private static void writeList(String str, List<?> list, JSONObject jSONObject) throws JSONException, IllegalAccessException {
        ...
        for (int i9 = 0; i9 < list.size(); i9++) {
            disposeType(LIST_ITEM_VALUE + i9, list.get(i9), jSONObject2);
            if (list.get(i9) instanceof IMessageEntity) {
