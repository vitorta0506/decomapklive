...
final class EANManufacturerOrgSupport {
    ...
    private synchronized void initIfNeeded() {
        if (this.ranges.isEmpty()) {
            ...
            add(new int[]{60, TPOptionalID.OPTION_ID_BEFORE_LONG_BUFFER_STRATEGY}, "US/CA");
            ...
            add(new int[]{880}, "KR");
            add(new int[]{885}, "TH");
            add(new int[]{Photo_Dialog_Fragment.REQUEST_CODE}, "SG");
            add(new int[]{890}, "IN");
            add(new int[]{893}, "VN");
            add(new int[]{896}, "PK");
            add(new int[]{899}, "ID");
            add(new int[]{900, VKApiCodes.CODE_CHAT_INVITE_MAKE_LINK_DENIED}, "AT");
            add(new int[]{930, VKApiCodes.CODE_CHAT_MR_ALREADY_SEND}, "AU");
    ...
    public String lookupCountryIdentifier(String str) {
        ...
        for (int i10 = 0; i10 < size && parseInt >= (i9 = (iArr = this.ranges.get(i10))[0]); i10++) {
            if (iArr.length != 1) {
                i9 = iArr[1];
            }
            if (parseInt <= i9) {
                return this.countryIdentifiers.get(i10);
            }
        }
        return null;
    }
