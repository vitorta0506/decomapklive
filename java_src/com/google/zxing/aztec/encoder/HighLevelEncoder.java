...
public final class HighLevelEncoder {
    ...
    static {
        ...
        for (int i9 = 65; i9 <= 90; i9++) {
            CHAR_MAP[0][i9] = (i9 - 65) + 2;
        }
        CHAR_MAP[1][32] = 1;
        for (int i10 = 97; i10 <= 122; i10++) {
            CHAR_MAP[1][i10] = (i10 - 97) + 2;
        }
        CHAR_MAP[2][32] = 1;
        for (int i11 = 48; i11 <= 57; i11++) {
            CHAR_MAP[2][i11] = (i11 - 48) + 2;
        }
        int[][] iArr2 = CHAR_MAP;
        iArr2[2][44] = 12;
        iArr2[2][46] = 13;
    ...
    private void updateStateForChar(State state, int i9, Collection<State> collection) {
        char c10 = (char) (this.text[i9] & UByte.MAX_VALUE);
        boolean z10 = CHAR_MAP[state.getMode()][c10] > 0;
        State state2 = null;
        for (int i10 = 0; i10 <= 4; i10++) {
            int i11 = CHAR_MAP[i10][c10];
            if (i11 > 0) {
                if (state2 == null) {
                    state2 = state.endBinaryShift(i9);
                }
                if (!z10 || i10 == state.getMode() || i10 == 2) {
                    collection.add(state2.latchAndAppend(i10, i11));
                }
                if (!z10 && SHIFT_TABLE[state.getMode()][i10] >= 0) {
                    collection.add(state2.shiftAndAppend(i10, i11));
                }
            }
        }
        if (state.getBinaryShiftByteCount() > 0 || CHAR_MAP[state.getMode()][c10] == 0) {
            collection.add(state.addBinaryShiftChar(i9));
        }
    }

    private static void updateStateForPair(State state, int i9, int i10, Collection<State> collection) {
        State endBinaryShift = state.endBinaryShift(i9);
        collection.add(endBinaryShift.latchAndAppend(4, i10));
        if (state.getMode() != 4) {
            collection.add(endBinaryShift.shiftAndAppend(4, i10));
        }
        if (i10 == 3 || i10 == 4) {
            collection.add(endBinaryShift.latchAndAppend(2, 16 - i10).latchAndAppend(2, 1));
        }
        if (state.getBinaryShiftByteCount() > 0) {
            collection.add(state.addBinaryShiftChar(i9).addBinaryShiftChar(i9 + 1));
        }
    }

    private Collection<State> updateStateListForChar(Iterable<State> iterable, int i9) {
        LinkedList linkedList = new LinkedList();
        for (State state : iterable) {
            updateStateForChar(state, i9, linkedList);
        }
        return simplifyStates(linkedList);
    }

    private static Collection<State> updateStateListForPair(Iterable<State> iterable, int i9, int i10) {
        LinkedList linkedList = new LinkedList();
        for (State state : iterable) {
            updateStateForPair(state, i9, i10, linkedList);
        }
        return simplifyStates(linkedList);
    }
