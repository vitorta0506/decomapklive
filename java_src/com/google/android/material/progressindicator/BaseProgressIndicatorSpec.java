...
public abstract class BaseProgressIndicatorSpec {
    ...
    public BaseProgressIndicatorSpec(@NonNull Context context, @Nullable AttributeSet attributeSet, @AttrRes int i9, @StyleRes int i10) {
        int dimensionPixelSize = context.getResources().getDimensionPixelSize(R.dimen.mtrl_progress_track_thickness);
        TypedArray obtainStyledAttributes = ThemeEnforcement.obtainStyledAttributes(context, attributeSet, R.styleable.BaseProgressIndicator, i9, i10, new int[0]);
    ...
    private void loadIndicatorColors(@NonNull Context context, @NonNull TypedArray typedArray) {
        ...
        } else if (typedArray.peekValue(i9).type != 1) {
            this.indicatorColors = new int[]{typedArray.getColor(i9, -1)};
        } else {
    ...
    private void loadTrackColor(@NonNull Context context, @NonNull TypedArray typedArray) {
        int i9 = R.styleable.BaseProgressIndicator_trackColor;
        if (typedArray.hasValue(i9)) {
            this.trackColor = typedArray.getColor(i9, -1);
            return;
        }
