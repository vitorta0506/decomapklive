...
class ClockFaceView extends RadialViewGroup implements ClockHandView.OnRotateListener {
    ...
    private void findIntersectingTextView() {
        ...
        for (int i9 = 0; i9 < this.textViewPool.size(); i9++) {
    ...
    private void updateTextViews(@StringRes int i9) {
        ...
        for (int i10 = 0; i10 < Math.max(this.values.length, size); i10++) {
            ...
            if (i10 >= this.values.length) {
                ...
                if (i9 != 0) {
                    textView.setContentDescription(getResources().getString(i9, this.values[i10]));
                }
            }
        }
    }
