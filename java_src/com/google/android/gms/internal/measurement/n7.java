package com.google.android.gms.internal.measurement;

import java.util.AbstractList;
import java.util.Arrays;
import java.util.Collection;
import java.util.RandomAccess;
/* loaded from: classes2.dex */
final class n7 extends j7 implements RandomAccess, ga {

    /* renamed from: d  reason: collision with root package name */
    private static final n7 f8224d;

    /* renamed from: b  reason: collision with root package name */
    private boolean[] f8225b;

    /* renamed from: c  reason: collision with root package name */
    private int f8226c;

    static {
        n7 n7Var = new n7(new boolean[0], 0);
        f8224d = n7Var;
        n7Var.D();
    }

    n7() {
        this(new boolean[10], 0);
    }

    private final String f(int i9) {
        int i10 = this.f8226c;
        return "Index:" + i9 + ", Size:" + i10;
    }

    private final void g(int i9) {
        if (i9 < 0 || i9 >= this.f8226c) {
            throw new IndexOutOfBoundsException(f(i9));
        }
    }

    @Override // java.util.AbstractList, java.util.List
    public final /* synthetic */ void add(int i9, Object obj) {
        int i10;
        boolean booleanValue = ((Boolean) obj).booleanValue();
        c();
        if (i9 >= 0 && i9 <= (i10 = this.f8226c)) {
            boolean[] zArr = this.f8225b;
            if (i10 < zArr.length) {
                System.arraycopy(zArr, i9, zArr, i9 + 1, i10 - i9);
            } else {
                boolean[] zArr2 = new boolean[((i10 * 3) / 2) + 1];
                System.arraycopy(zArr, 0, zArr2, 0, i9);
                System.arraycopy(this.f8225b, i9, zArr2, i9 + 1, this.f8226c - i9);
                this.f8225b = zArr2;
            }
            this.f8225b[i9] = booleanValue;
            this.f8226c++;
            ((AbstractList) this).modCount++;
            return;
        }
        throw new IndexOutOfBoundsException(f(i9));
    }

    @Override // com.google.android.gms.internal.measurement.j7, java.util.AbstractCollection, java.util.Collection, java.util.List
    public final boolean addAll(Collection collection) {
        c();
        e9.e(collection);
        if (!(collection instanceof n7)) {
            return super.addAll(collection);
        }
        n7 n7Var = (n7) collection;
        int i9 = n7Var.f8226c;
        if (i9 == 0) {
            return false;
        }
        int i10 = this.f8226c;
        if (Integer.MAX_VALUE - i10 >= i9) {
            int i11 = i10 + i9;
            boolean[] zArr = this.f8225b;
            if (i11 > zArr.length) {
                this.f8225b = Arrays.copyOf(zArr, i11);
            }
            System.arraycopy(n7Var.f8225b, 0, this.f8225b, this.f8226c, n7Var.f8226c);
            this.f8226c = i11;
            ((AbstractList) this).modCount++;
            return true;
        }
        throw new OutOfMemoryError();
    }

    @Override // java.util.AbstractCollection, java.util.Collection, java.util.List
    public final boolean contains(Object obj) {
        return indexOf(obj) != -1;
    }

    public final void d(boolean z10) {
        c();
        int i9 = this.f8226c;
        boolean[] zArr = this.f8225b;
        if (i9 == zArr.length) {
            boolean[] zArr2 = new boolean[((i9 * 3) / 2) + 1];
            System.arraycopy(zArr, 0, zArr2, 0, i9);
            this.f8225b = zArr2;
        }
        boolean[] zArr3 = this.f8225b;
        int i10 = this.f8226c;
        this.f8226c = i10 + 1;
        zArr3[i10] = z10;
    }

    @Override // com.google.android.gms.internal.measurement.j7, java.util.AbstractList, java.util.Collection, java.util.List
    public final boolean equals(Object obj) {
        if (this == obj) {
            return true;
        }
        if (!(obj instanceof n7)) {
            return super.equals(obj);
        }
        n7 n7Var = (n7) obj;
        if (this.f8226c != n7Var.f8226c) {
            return false;
        }
        boolean[] zArr = n7Var.f8225b;
        for (int i9 = 0; i9 < this.f8226c; i9++) {
            if (this.f8225b[i9] != zArr[i9]) {
                return false;
            }
        }
        return true;
    }

    @Override // java.util.AbstractList, java.util.List
    public final /* synthetic */ Object get(int i9) {
        g(i9);
        return Boolean.valueOf(this.f8225b[i9]);
    }

    @Override // com.google.android.gms.internal.measurement.j7, java.util.AbstractList, java.util.Collection, java.util.List
    public final int hashCode() {
        int i9 = 1;
        for (int i10 = 0; i10 < this.f8226c; i10++) {
            i9 = (i9 * 31) + e9.a(this.f8225b[i10]);
        }
        return i9;
    }

    @Override // java.util.AbstractList, java.util.List
    public final int indexOf(Object obj) {
        if (obj instanceof Boolean) {
            boolean booleanValue = ((Boolean) obj).booleanValue();
            int i9 = this.f8226c;
            for (int i10 = 0; i10 < i9; i10++) {
                if (this.f8225b[i10] == booleanValue) {
                    return i10;
                }
            }
            return -1;
        }
        return -1;
    }

    @Override // com.google.android.gms.internal.measurement.d9
    public final /* bridge */ /* synthetic */ d9 j(int i9) {
        if (i9 >= this.f8226c) {
            return new n7(Arrays.copyOf(this.f8225b, i9), this.f8226c);
        }
        throw new IllegalArgumentException();
    }

    @Override // com.google.android.gms.internal.measurement.j7, java.util.AbstractList, java.util.List
    public final /* bridge */ /* synthetic */ Object remove(int i9) {
        int i10;
        c();
        g(i9);
        boolean[] zArr = this.f8225b;
        boolean z10 = zArr[i9];
        if (i9 < this.f8226c - 1) {
            System.arraycopy(zArr, i9 + 1, zArr, i9, (i10 - i9) - 1);
        }
        this.f8226c--;
        ((AbstractList) this).modCount++;
        return Boolean.valueOf(z10);
    }

    @Override // java.util.AbstractList
    protected final void removeRange(int i9, int i10) {
        c();
        if (i10 >= i9) {
            boolean[] zArr = this.f8225b;
            System.arraycopy(zArr, i10, zArr, i9, this.f8226c - i10);
            this.f8226c -= i10 - i9;
            ((AbstractList) this).modCount++;
            return;
        }
        throw new IndexOutOfBoundsException("toIndex < fromIndex");
    }

    @Override // java.util.AbstractList, java.util.List
    public final /* bridge */ /* synthetic */ Object set(int i9, Object obj) {
        boolean booleanValue = ((Boolean) obj).booleanValue();
        c();
        g(i9);
        boolean[] zArr = this.f8225b;
        boolean z10 = zArr[i9];
        zArr[i9] = booleanValue;
        return Boolean.valueOf(z10);
    }

    @Override // java.util.AbstractCollection, java.util.Collection, java.util.List
    public final int size() {
        return this.f8226c;
    }

    private n7(boolean[] zArr, int i9) {
        this.f8225b = zArr;
        this.f8226c = i9;
    }

    @Override // com.google.android.gms.internal.measurement.j7, java.util.AbstractList, java.util.AbstractCollection, java.util.Collection, java.util.List
    public final /* bridge */ /* synthetic */ boolean add(Object obj) {
        d(((Boolean) obj).booleanValue());
        return true;
    }
}
