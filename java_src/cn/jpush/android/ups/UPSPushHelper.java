...
public class UPSPushHelper {
    ...
    public static void createMapping(Context context, long j10, ICallbackResult iCallbackResult) {
        init(context);
        if (iCallbackResult == null) {
            Logger.e(TAG, "requestParams is null");
        } else if (cacheMap.containsKey(Long.valueOf(j10))) {
            Logger.v(TAG, "rid " + j10 + " has exist.");
        } else if (!(iCallbackResult instanceof Activity)) {
            cacheMap.put(Long.valueOf(j10), iCallbackResult);
        } else {
            cacheMap.put(Long.valueOf(j10), new WeakReference(iCallbackResult));
        }
    }
    ...
    public static ICallbackResult removeMapping(long j10) {
        return objectToCallBack(cacheMap.remove(Long.valueOf(j10)));
    }

    public static void upsCallBack(Context context, int i9, String str, int i10, String str2) {
        try {
            Intent intent = new Intent();
            intent.setAction(context.getPackageName() + ACTION_UPS);
            intent.setPackage(context.getPackageName());
            intent.putExtra(KEY_RID, i9);
            ...
            Logger.ee(TAG, "[upsCallBack] failed:" + th2.getMessage());
        }
    }
}
