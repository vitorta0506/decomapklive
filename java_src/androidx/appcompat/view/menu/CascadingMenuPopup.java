...
final class CascadingMenuPopup extends MenuPopup implements View.OnKeyListener, PopupWindow.OnDismissListener {
        ...
        public void onItemHoverEnter(@NonNull final MenuBuilder menuBuilder, @NonNull final MenuItem menuItem) {
            ...
            while (true) {
                if (i9 >= size) {
                    i9 = -1;
                    break;
                } else if (menuBuilder == CascadingMenuPopup.this.mShowingMenus.get(i9).menu) {
                    break;
                } else {
                    i9++;
                }
            ...
            int i10 = i9 + 1;
            final CascadingMenuInfo cascadingMenuInfo = i10 < CascadingMenuPopup.this.mShowingMenus.size() ? CascadingMenuPopup.this.mShowingMenus.get(i10) : null;
    ...
    public CascadingMenuPopup(@NonNull Context context, @NonNull View view, @AttrRes int i9, @StyleRes int i10, boolean z10) {
    ...
    private int findIndexOfAddedMenu(@NonNull MenuBuilder menuBuilder) {
        ...
        for (int i9 = 0; i9 < size; i9++) {
            if (menuBuilder == this.mShowingMenus.get(i9).menu) {
                return i9;
            }
        }
        return -1;
    ...
    private View findParentViewForSubmenu(@NonNull CascadingMenuInfo cascadingMenuInfo, @NonNull MenuBuilder menuBuilder) {
        ...
        while (true) {
            if (i10 >= count) {
                i10 = -1;
                break;
            } else if (findMenuItemForSubmenu == menuAdapter.getItem(i10)) {
                break;
            } else {
                i10++;
            }
        }
        if (i10 != -1 && (firstVisiblePosition = (i10 + i9) - listView.getFirstVisiblePosition()) >= 0 && firstVisiblePosition < listView.getChildCount()) {
    ...
    private int getNextMenuPosition(int i9) {
        ...
        return this.mLastPosition == 1 ? (iArr[0] + listView.getWidth()) + i9 > rect.right ? 0 : 1 : iArr[0] - i9 < 0 ? 1 : 0;
    }
        ...
        if (view != null) {
            ...
            if ((this.mDropDownGravity & 5) == 5) {
                if (!z10) {
                    ...
                    i11 = i9 - measureIndividualMenuWidth;
                }
                i11 = i9 + measureIndividualMenuWidth;
            } else {
                if (z10) {
            ...
            createPopupWindow.setHorizontalOffset(i11);
            createPopupWindow.setOverlapAnchor(true);
            createPopupWindow.setVerticalOffset(i10);
        } else {
            if (this.mHasXOffset) {
    ...
    public void onCloseMenu(MenuBuilder menuBuilder, boolean z10) {
        ...
        int i9 = findIndexOfAddedMenu + 1;
        if (i9 < this.mShowingMenus.size()) {
            this.mShowingMenus.get(i9).menu.close(false);
        }
    ...
    public void onDismiss() {
        ...
        while (true) {
            ...
            cascadingMenuInfo = this.mShowingMenus.get(i9);
            if (!cascadingMenuInfo.window.isShowing()) {
                break;
            }
            i9++;
    ...
    public void setGravity(int i9) {
        if (this.mRawDropDownGravity != i9) {
            ...
            this.mDropDownGravity = GravityCompat.getAbsoluteGravity(i9, ViewCompat.getLayoutDirection(this.mAnchorView));
        }
    }
