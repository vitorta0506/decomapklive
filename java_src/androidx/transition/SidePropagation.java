...
public class SidePropagation extends VisibilityPropagation {
    ...
    */
    ...
    private int getMaxDistance(ViewGroup viewGroup) {
        int i9 = this.mSide;
        if (i9 != 3 && i9 != 5 && i9 != 8388611 && i9 != 8388613) {
    ...
    public long getStartDelay(ViewGroup viewGroup, Transition transition, TransitionValues transitionValues, TransitionValues transitionValues2) {
        ...
        float distance = distance(viewGroup, viewX, viewY, i10, i11, round, round2, width, height) / getMaxDistance(viewGroup);
        long duration = transition.getDuration();
        if (duration < 0) {
            duration = 300;
        }
        return Math.round((((float) (duration * i9)) / this.mPropagationSpeed) * distance);
    }
