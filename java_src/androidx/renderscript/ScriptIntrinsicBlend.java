...
public class ScriptIntrinsicBlend extends ScriptIntrinsic {
    ...
    ScriptIntrinsicBlend(long j10, RenderScript renderScript) {
        super(j10, renderScript);
    }

    private void blend(int i9, Allocation allocation, Allocation allocation2) {
        if (allocation.getElement().isCompatible(Element.U8_4(this.mRS))) {
            if (allocation2.getElement().isCompatible(Element.U8_4(this.mRS))) {
                forEach(i9, allocation, allocation2, (FieldPacker) null);
                return;
            }
