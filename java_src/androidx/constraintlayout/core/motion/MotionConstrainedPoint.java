...
class MotionConstrainedPoint implements Comparable<MotionConstrainedPoint> {
    ...
    public void addValues(HashMap<String, SplineSet> hashMap, int i9) {
        for (String str : hashMap.keySet()) {
            ...
            switch (c10) {
                case 0:
                    splineSet.setPoint(i9, Float.isNaN(this.rotationX) ? 0.0f : this.rotationX);
                    break;
                case 1:
                    splineSet.setPoint(i9, Float.isNaN(this.rotationY) ? 0.0f : this.rotationY);
                    break;
                case 2:
                    splineSet.setPoint(i9, Float.isNaN(this.rotation) ? 0.0f : this.rotation);
                    break;
                case 3:
                    splineSet.setPoint(i9, Float.isNaN(this.translationX) ? 0.0f : this.translationX);
                    break;
                case 4:
                    splineSet.setPoint(i9, Float.isNaN(this.translationY) ? 0.0f : this.translationY);
                    break;
                case 5:
                    splineSet.setPoint(i9, Float.isNaN(this.translationZ) ? 0.0f : this.translationZ);
                    break;
                case 6:
                    splineSet.setPoint(i9, Float.isNaN(this.mProgress) ? 0.0f : this.mProgress);
                    break;
                case 7:
                    splineSet.setPoint(i9, Float.isNaN(this.mPivotX) ? 0.0f : this.mPivotX);
                    break;
                case '\b':
                    splineSet.setPoint(i9, Float.isNaN(this.mPivotY) ? 0.0f : this.mPivotY);
                    break;
                case '\t':
                    splineSet.setPoint(i9, Float.isNaN(this.scaleX) ? 1.0f : this.scaleX);
                    break;
                case '\n':
                    splineSet.setPoint(i9, Float.isNaN(this.scaleY) ? 1.0f : this.scaleY);
                    break;
                case 11:
                    splineSet.setPoint(i9, Float.isNaN(this.alpha) ? 1.0f : this.alpha);
                    break;
                case '\f':
                    splineSet.setPoint(i9, Float.isNaN(this.mPathRotate) ? 0.0f : this.mPathRotate);
                    break;
                default:
    ...
    public void different(MotionConstrainedPoint motionConstrainedPoint, HashSet<String> hashSet) {
        ...
        if (i9 != i10 && this.mVisibilityMode == 0 && (i9 == 4 || i10 == 4)) {
            hashSet.add("alpha");
        }
    ...
    void fillStandard(double[] dArr, int[] iArr) {
        float[] fArr = {this.position, this.f806x, this.f807y, this.width, this.height, this.alpha, this.elevation, this.rotation, this.rotationX, this.rotationY, this.scaleX, this.scaleY, this.mPivotX, this.mPivotY, this.translationX, this.translationY, this.translationZ, this.mPathRotate};
        int i9 = 0;
    ...
    public void setState(Rect rect, MotionWidget motionWidget, int i9, float f10) {
    ...
    void different(MotionConstrainedPoint motionConstrainedPoint, boolean[] zArr, String[] strArr) {
        ...
        zArr[1] = zArr[1] | diff(this.f806x, motionConstrainedPoint.f806x);
        zArr[2] = zArr[2] | diff(this.f807y, motionConstrainedPoint.f807y);
