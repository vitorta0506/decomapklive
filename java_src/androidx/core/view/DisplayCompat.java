...
public final class DisplayCompat {
    ...
    private static Point parseDisplaySize(@NonNull String str) throws NumberFormatException {
        String[] split = str.trim().split(x.f19108w, -1);
        if (split.length == 2) {
    ...
    public static class Api23Impl {
        ...
        public static ModeCompat[] getSupportedModes(@NonNull Context context, @NonNull Display display) {
            ...
            if (currentDisplaySizeFromWorkarounds != null && !physicalSizeEquals(mode, currentDisplaySizeFromWorkarounds)) {
                for (int i9 = 0; i9 < supportedModes.length; i9++) {
                    if (physicalSizeEquals(supportedModes[i9], mode)) {
                        modeCompat = new ModeCompat(supportedModes[i9], currentDisplaySizeFromWorkarounds);
                    } else {
                        modeCompat = new ModeCompat(supportedModes[i9], false);
                    }
                    modeCompatArr[i9] = modeCompat;
                }
            } else {
        ...
        static boolean isCurrentModeTheLargestMode(@NonNull Display display) {
            ...
            Display.Mode[] supportedModes = display.getSupportedModes();
            for (int i9 = 0; i9 < supportedModes.length; i9++) {
                if (mode.getPhysicalHeight() < supportedModes[i9].getPhysicalHeight() || mode.getPhysicalWidth() < supportedModes[i9].getPhysicalWidth()) {
                    return false;
                }
            }
            return true;
