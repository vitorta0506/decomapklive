...
public final class PackageInfoCompat {
    ...
    public static class Api28Impl {
        ...
        static boolean hasSigningCertificate(@NonNull PackageManager packageManager, @NonNull String str, @NonNull byte[] bArr, int i9) {
        ...
        if (signatures.size() != 0 && map.size() <= signatures.size() && (!z10 || map.size() == signatures.size())) {
            ...
            if (map.containsValue(1)) {
                ...
                for (int i9 = 0; i9 < signatures.size(); i9++) {
                    bArr3[i9] = computeSHA256Digest(signatures.get(i9).toByteArray());
                }
            }
