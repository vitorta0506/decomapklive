...
public final class ColorStateListInflaterCompat {
    ...
    @Nullable
    public static ColorStateList inflate(@NonNull Resources resources, @XmlRes int i9, @Nullable Resources.Theme theme) {
        try {
            return createFromXml(resources, resources.getXml(i9), theme);
        } catch (Exception e10) {
    ...
    @ColorInt
    private static int modulateColorAlpha(@ColorInt int i9, @FloatRange(from = 0.0d, to = 1.0d) float f10, @FloatRange(from = 0.0d, to = 100.0d) float f11) {
        boolean z10 = f11 >= 0.0f && f11 <= 100.0f;
        if (f10 != 1.0f || z10) {
            int clamp = MathUtils.clamp((int) ((Color.alpha(i9) * f10) + 0.5f), 0, 255);
            if (z10) {
                CamColor fromColor = CamColor.fromColor(i9);
                i9 = CamColor.toColor(fromColor.getHue(), fromColor.getChroma(), f11);
            }
            return (i9 & ViewCompat.MEASURED_SIZE_MASK) | (clamp << 24);
        }
        return i9;
    }
