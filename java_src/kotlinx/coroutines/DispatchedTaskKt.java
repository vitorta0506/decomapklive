...
public final class DispatchedTaskKt {
    ...
    public static final <T> void dispatch(@NotNull DispatchedTask<? super T> dispatchedTask, int i9) {
        Continuation<? super T> delegate$kotlinx_coroutines_core = dispatchedTask.getDelegate$kotlinx_coroutines_core();
        boolean z10 = i9 == 4;
        if (!z10 && (delegate$kotlinx_coroutines_core instanceof DispatchedContinuation) && isCancellableMode(i9) == isCancellableMode(dispatchedTask.resumeMode)) {
            CoroutineDispatcher coroutineDispatcher = ((DispatchedContinuation) delegate$kotlinx_coroutines_core).dispatcher;
            CoroutineContext context = delegate$kotlinx_coroutines_core.getContext();
            if (coroutineDispatcher.isDispatchNeeded(context)) {
                coroutineDispatcher.mo3203dispatch(context, dispatchedTask);
                return;
            } else {
    ...
    public static final <T> void resume(@NotNull DispatchedTask<? super T> dispatchedTask, @NotNull Continuation<? super T> continuation, boolean z10) {
        ...
        Object m1657constructorimpl = Result.m1657constructorimpl(successfulResult$kotlinx_coroutines_core);
        if (z10) {
            ...
            try {
                dispatchedContinuation.continuation.resumeWith(m1657constructorimpl);
